  ...
  for (auto it1 = entity_list.begin(); it1 != entity_list.end(); ++it1)
    {
      ...
      if(!((*it1)->hasComponent<BoxCollider>()))
      {
        continue;
      }
      
      BoxCollider& a1 = (*it1)->getComponent<BoxCollider>();
      for (auto it2 = std::next(it1); it2 != entity_list.end(); ++it2)
        {
          if(!((*it2)->hasComponent<BoxCollider>()))
          {
            continue;
          }
          BoxCollider& a2 = (*it2)->getComponent<BoxCollider>();
          ...
          if ((a1).m_box.getGlobalBounds().intersects((a2).m_box.getGlobalBounds()))
          {
            ...
            sf::Vector2f direction = a1.m_trans.m_pos - a2.m_trans.m_pos;
